...
import { homeAPI, homeAPI_1, DisplayQueue, DisplayQueueMember } from './hp_data.js';
...
export const init_card_display_1 = function (card_display_node = new HTMLElement(), search_text = "") {
    if(EL_KW_LIST.length > 0) {
        EMPTY_ARRAY(_CARD_DISPLAY_NODES);
        EMPTY_ARRAY(EL_KW_LIST);
    }
    ...
    create_display(homeAPI_1([search_text], true), [], card_display_node);
...
export const EL_callback_1 = function (btn, btn_queue) {
    const CARD_DISPLAY_NODE = document.querySelector("#card-display-ex");
    ...
    create_display(homeAPI_1(EL_KW_LIST, false, true), _CARD_DISPLAY_NODES, CARD_DISPLAY_NODE);
    // $(CARD_DISPLAY_NODE).animate({scrollTop: 1}, 1);
    $(CARD_DISPLAY_NODE.parentElement).animate({scrollTop: 1}, 480);
}
