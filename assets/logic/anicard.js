import { VNS_panel, VNS_panel_1, EL_panel, EL_panel_1 } from './hp_panel.js';
import { MNav, Collapse_Btn } from './hp_mobileNav.js';
import { init_card_display } from './hp_middle.js';
import { init_card_display_1 } from './hp_middle.js';
...
function openHomepage_ex_1() {
    $.ajax({
        url: "./assets/static/homepage_ex_1.html",  // 你复制的文件
        type: "get",
        contentType: "text/html",
        dataType: "html",
        success: function(res) {
            document.querySelector("main").innerHTML = res;
            Homepage_ex_1_loading();  // 直接复用原有加载逻辑
        }
    });
}
...
// 左侧导航顺序
function Homepage_ex_1_loading() {
    ...
    /* 这一行是上面的category部分 */
    VNS_panel_1.appendTo(sidebar_node, vns_extraNode_html, vns_extraClass_toA_arr, vns_extraAttribute_toA, vns_method_to_btn_name);

    // Chart_panel.appendTo(sidebar_node, "", chart_extraClass_toA_arr, {});

    /* 这一行是下面的多选部分 */
    // EL_panel_1.appendTo(sidebar_node, "", el_extraClass_toA_arr, {});

    const collapse_btn = new Collapse_Btn('.mobile-nav-btn', '#sidebar-ex', '.mobile-nav-scrollSpy');
    init_card_display_1(card_display_node);
    /* 搜索筛选功能 */
    // searchBox_EventListener(card_display_node);
    modal_EventListener();
