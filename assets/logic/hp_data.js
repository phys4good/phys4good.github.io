...
const homepage_vns_url_1 = "./assets/json/vns_collection_1.json";
const homepage_cards_url_1 = "./assets/json/anicard_dataset_1.json";
...
const VISIBLE_DISPLAY_QUEUE_1 = new DisplayQueue("visible display queue 1");
...
function _create_display_subjects (display_url, displayQueue) {
    ...
    $.getJSON(display_url, json => {
        ...
        json.forEach((json_item, i, jsonArr) => {
            ...
            displayQueue.append(display_reminder_subject, is_first);
        });
    });
}
...
const init_display_container = function (keywords = [], search = false, EL_filter = false) {
    _create_display_subjects (homepage_vns_url, VISIBLE_DISPLAY_QUEUE);
...
const init_display_container_1 = function (keywords = [], search = false, EL_filter = false) {
    _create_display_subjects (homepage_vns_url_1, VISIBLE_DISPLAY_QUEUE_1);
    _create_card_subjects(homepage_cards_url_1, VISIBLE_DISPLAY_QUEUE_1);
    if(!search && !EL_filter ) {
        return VISIBLE_DISPLAY_QUEUE_1;
    }

    if(search == true) {
        return VISIBLE_DISPLAY_QUEUE_1.card_filter(FILTER_KEYWORD_QUEUE, keywords, true);
    }

    if(EL_filter == true) {
        return VISIBLE_DISPLAY_QUEUE_1.card_filter(FILTER_TAG_QUEUE, keywords);
    }
...
export { 
    ...
    init_display_container_1 as homeAPI_1
};
